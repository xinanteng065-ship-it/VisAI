...
def generate_ai_summary(news_text, category):
    ...
    4. それぞれのニュースに、評価している意見、批判している意見、中立的な意見などもソースと共にわかりやすく提示して欲しい！
    5. 記事のURLは要約内には含めず、文章のみで構成してください(URLは別途付与するため)。
    6. 冒頭に「お疲れ様です!」など、読む人に寄り添う挨拶を入れてください。
    7. それぞれのニュースタイトルの前に数字をつけてください。その後にニュースを解説してほしいです。
